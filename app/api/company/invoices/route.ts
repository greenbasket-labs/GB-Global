...
export async function POST(req:Request){
  ...
  if(!organizationId||!amountKobo||!dueDate)return NextResponse.json({error:'Organization, amount and due date are required.'},{status:400});
...
export async function PATCH(req:Request){
  ...
  await prisma.auditLog.create({data:{actorId:u.id,action:'INVOICE_STATUS_CHANGED',entity:'Invoice',entityId:String(invoiceNumber),metadata:{invoiceNumber,status,changedAt:new Date().toISOString()}}});
