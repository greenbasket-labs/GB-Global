...
export async function POST(req:Request){
  ...
  const o=await getCurrentOrganization(u);
  ...
  const cs=await prisma.clientService.upsert({where:{organizationId_serviceId:{organizationId:o.id,serviceId:id}},update:{status:existing?.status==='ACTIVE'?'ACTIVE':'REQUESTED',billingCycle,priceKobo},create:{organizationId:o.id,serviceId:id,status:'REQUESTED',billingCycle,priceKobo}});
  await prisma.auditLog.create({data:{actorId:u.id,action:'SERVICE_REQUESTED',entity:'ClientService',entityId:cs.id,metadata:{organizationId:o.id,serviceId:id,serviceName:s.name}}});
