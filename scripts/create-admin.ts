...
async function main(){
  ...
  const user=await prisma.user.upsert({
    where:{email},
    update:{name,passwordHash,role:UserRole.COMPANY_ADMIN,emailVerifiedAt:new Date()},
    create:{name,email,passwordHash,role:UserRole.COMPANY_ADMIN,emailVerifiedAt:new Date()}
  });
