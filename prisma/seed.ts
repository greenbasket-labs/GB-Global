...
async function main(){
 for(const [name,description,category,oneTime,monthly,yearly] of services){const slug=name.toLowerCase().replace(/[^a-z0-9]+/g,'-');const active=name==='School Management System';await prisma.service.upsert({where:{slug},update:{name,description,category:category as any,oneTimePriceKobo:oneTime,monthlyPriceKobo:monthly,yearlyPriceKobo:yearly,active},create:{name,slug,description,category:category as any,oneTimePriceKobo:oneTime,monthlyPriceKobo:monthly,yearlyPriceKobo:yearly,active}})}
